...
public class CheckoutController {
    ...
    @Operation(
            summary = "Tạo giao dịch thanh toán chi phí gửi xe",
            description = "Ghi nhận giao dịch thanh toán tiền gửi xe, tiền cọc hoặc tiền phạt mất thẻ."
    )
    @ApiResponses(value = {
            @ApiResponse(
                    responseCode = "200",
    ...
    public ResponseEntity<PaymentResponse> createPayment(
        ...
        PaymentResponse response = checkoutService.createPayment(request);
        return ResponseEntity.ok(response);
    }
